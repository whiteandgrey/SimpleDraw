...
**从 Obsidian 插件同步 v1.5.6——标签层级跟随箭头（仅创建时 insertBefore）**

### Bug 修复

- **标签层级不再跟随箭头**：v1.5.4 为修复双击而删除 `insertBefore`，导致标签退回到 `createDiv` 默认位置（`elementsLayer` 末尾），置顶/置底文本框后标签仍在最上方

### 修复

将 `insertBefore` 移入标签**首次创建**的代码块内，创建时定位到所连最高 z-order 文本框后面，后续帧不再移动 DOM，双击和层级同时正确。

### 文件变更

| 文件 | 改动 | 说明 |
|------|------|------|
| `src/renderer/canvas-view.ts` | +14 行 | insertBefore 移入 label 创建块内 |
| `package.json` | +1 行 / -1 行 | 版本号 1.5.5 → 1.5.6 |
| `scripts/installer.nsi` | 4 处 | 版本号同步为 1.5.6 |
